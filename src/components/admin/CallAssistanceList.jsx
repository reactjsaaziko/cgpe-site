...
const CallAssistanceList = () => {
    ...
    const fetchCalls = async (page = 1) => {
        try {
            ...
            setError(err.response?.data?.message || 'Failed to fetch call assistance requests');
            setCalls([]);
            setPagination({});
        } finally {
            setLoading(false);
    ...
    const handleDelete = async (callId) => {
        if (!window.confirm('Are you sure you want to delete this call assistance request?')) return;
    ...
    const handleAddNote = async () => {
        ...
        try {
            const token = localStorage.getItem('adminToken');
            await axios.post(`${API_BASE_URL}/api/call-assistance/admin/${selectedCall._id}/notes`,
                { content: noteContent },
                { headers: { Authorization: `Bearer ${token}` } }
            );
                    ...
                    <>
                        ...
                        {calls.length === 0 && (
                            <div className="bg-white border border-gray-100 rounded-xl p-8 text-center shadow-sm">
                                ...
                                <div className="text-lg font-semibold text-gray-800">No call assistance requests found</div>
...
export default CallAssistanceList;
