...
const HealthInsuranceBulkUpload = ({ onClose, onSuccess, defaultPolicyType }) => {
    ...
    const downloadTemplate = () => {
        ...
        const sample = `${headers.join(',')}\nExample Health Policy,${defaultPolicyType || 'individual'},500000,5 Lakhs,8000,yearly,1,18,65,Sample health description,Sample Company,https://logo.png,4.5,10,true,false,0,0,30,95,5000,no-limit,once-a-year,false,false,false,false,true,95% of claims paid within 3 months*,1 more plan,"Feature A|Feature B","Benefit A|Benefit B","Exclusion A|Exclusion B","Doc A|Doc B","tag1|tag2","badge1|badge2"`;
        const blob = new Blob([sample], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', 'health_policies_template.csv');
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/health-insurance/policies/bulk-upload', {
                method: 'POST',
                body: formData
            });
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-white/20 overflow-hidden">
                <div className="p-6 text-black">
                    <div className="flex items-center justify-between">
                        <h3 className="text-2xl font-bold">Bulk Upload Health Insurance Policies</h3>
                ...
                <div className="p-6 space-y-4">                                                                                                            
                    <p className="text-sm text-gray-700">Upload a CSV (preferred) or JSON file. Arrays like features, benefits, exclusions, documents, tags, badges can be pipe-separated (e.g., Feature A|Feature B).</p>
...
HealthInsuranceBulkUpload.displayName = 'HealthInsuranceBulkUpload';

export default HealthInsuranceBulkUpload;
