...
export default function PolicySuggestList() {
    ...
    useEffect(() => {
        ...
        if (savedFormData) {
            try {
                ...
                // Show contact confirmation for restored data (but only if not already shown)
                if (!locationFormData) {
    ...
    useEffect(() => {
        if (locationFormData) {
            ...
            // Show contact confirmation when new form data is received
            setShowContactConfirmation(true);
        }
    }, [locationFormData]);
    ...
    useEffect(() => {
        if (formData) {
            ...
            // Map form data to filter parameters
            const filters = {
                smoker: formData.smokingStatus === 'Yes' ? 'yes' : 'no',
                alcohol: formData.alcoholStatus === 'Yes' ? 'yes' : 'no',
                age: age,
                education: formData.qualification === 'College graduate & above' ? 'graduate' : 
                          formData.qualification === '12th Pass' ? '12th-pass' : 'below-10th'
            };
    ...
    useEffect(() => {
        const fetchPolicies = async () => {
            try {
                ...
                // Add filter parameters based on user form data
                if (userFilters.smoker) {
                    params.append('smokerAllowed', userFilters.smoker === 'yes' ? 'true' : 'false');
                }
                if (userFilters.alcohol) {
                    params.append('alcoholAllowed', userFilters.alcohol === 'yes' ? 'true' : 'false');
                }
                if (userFilters.education) {
                    params.append('educationLevel', userFilters.education);
                }
                if (userFilters.age) {
                    params.append('maxAge', userFilters.age);
                }
                ...
                if (result.success) {
                    ...
                    const transformedPlans = result.data.map(policy => ({
                        _id: policy._id,
                        logo: policy.companyLogo || "./assets/images/default_logo.png",
                        title: policy.companyName,
                        plan: policy.policyName,
                        claim: `${policy.claimSettlementRatio}%`,
                        coverage: `${policy.policyTerm} Yrs`,
                        survival: policy.coverageAmountText || (policy.coverageAmount ? `₹${(policy.coverageAmount / 100000).toFixed(2)} L` : "₹0"),
                        death: policy.coverageAmountText || (policy.coverageAmount ? `₹${(policy.coverageAmount / 10000000).toFixed(0)} Cr` : "₹0"),
                        monthlyPrice: policy.premiumFrequency === 'monthly' ? `₹${policy.premiumAmount.toLocaleString()}` : `₹${Math.round(policy.premiumAmount / 12).toLocaleString()}`,
                        yearlyPrice: policy.premiumFrequency === 'yearly' ? `₹${policy.premiumAmount.toLocaleString()}` : `₹${(policy.premiumAmount * 12).toLocaleString()}`,
                        note: policy.discountPercentage > 0 ? `Online Saving ${policy.discountPercentage}%` : "",
                        highlights: policy.features?.slice(0, 2).map(feature => ({ 
                            label: feature, 
                            color: "bg-blue-50 text-blue-700" 
                        })) || [],
                        addons: policy.benefits?.length ? `${policy.benefits.length} Add-ons` : "No Add-ons",
                        offer: policy.discountPercentage > 0 ? `${policy.discountPercentage}% discount included` : "",
                        isYellow: policy.isPopular,
                        hasTag: policy.tags?.length > 0,
                        tag: policy.tags?.[0] || "",
                        ...
                        // New filter fields
                        smokerAllowed: policy.smokerAllowed,
                        alcoholAllowed: policy.alcoholAllowed,
                        educationLevel: policy.educationLevel,
    ...
    function PlanRow(p, i) {
        ...
        return (
            <div
                key={i}
                className={`rounded-2xl border ${p.isYellow ? "border-yellow-200 bg-yellow-50" : "border-gray-100 bg-white"} p-4 mb-3 shadow-sm`}
            >
                <div className="flex gap-4">
                    ...
                    <div className="min-w-[140px] text-right">
                        <button
                            ...
                            className="bg-blue-600 text-white font-bold text-lg rounded px-3 py-1 inline-block hover:bg-blue-700 transition-colors cursor-pointer"
                        >
                            {paymentMode === "yearly" ? p.yearlyPrice : p.monthlyPrice}
                            <span className="text-xs font-normal text-gray-200 ml-1">
                                {paymentMode === "yearly" ? "/year" : "/month"}
                            </span>
                        </button>
                        {p.note && <div className="text-xs text-green-600 mt-1">{p.note}</div>}
                        {paymentMode === "yearly" && (
                            <div className="text-xs text-green-600 mt-1 font-medium">
                                Save ₹{(parseInt(p.monthlyPrice.replace(/[^\d]/g, '')) * 12 - parseInt(p.yearlyPrice.replace(/[^\d]/g, ''))).toLocaleString()} yearly
                            </div>
                        )}
                ...
                <div className="flex items-center justify-between mt-2 text-xs">
                    ...
                    <div className="flex gap-2 items-center text-gray-500 rounded-lg px-3 py-1 bg-white relative">
                        {p.hasTag && (
                            <span className="bg-green-50 text-green-700 font-semibold rounded px-2 py-0.5 border border-green-200">{p.tag}</span>
                        )}
                        {/* Add-ons Dropdown */}
                        <div className="relative group  border border-gray-200 ">
                            <button className="flex items-center gap-1 hover:text-blue-600 focus:outline-none p-1">
                                <span>{p.addons}</span>
                                <svg className="w-3 h-3 ml-1 text-gray-400 group-hover:text-blue-600" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 20 20">
                            ...
                            <div className="absolute right-0 mt-2 w-44 bg-white border border-gray-200 rounded-lg shadow-lg z-10 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 pointer-events-none group-hover:pointer-events-auto group-focus-within:pointer-events-auto transition-opacity duration-150">
                                <ul className="py-2 text-sm text-gray-700">
                                    <li>
                                        <button className="w-full text-left px-4 py-2 hover:bg-blue-50">Accidental Death Benefit</button>
                                    </li>
                                    <li>
                                        <button className="w-full text-left px-4 py-2 hover:bg-blue-50">Critical Illness Cover</button>
                                    </li>
                                    <li>
                                        <button className="w-full text-left px-4 py-2 hover:bg-blue-50">Waiver of Premium</button>
                                    </li>
                                </ul>
                            </div>
                        </div>
    ...
    return (
        <div className="max-w-5xl mx-auto py-8">
            ...
            <div className="w-full flex flex-col gap-3 max-w-5xl mx-auto pt-6">
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                        <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-lg font-semibold text-yellow-800">Showing All Available Policies</h3>
                            </div>
                        </div>
                        <div className="text-sm text-yellow-700 mb-3">
                            No personal details provided. Please complete the form to get personalized policy recommendations.
                        </div>
                        <button 
                            onClick={() => window.location.href = '/terminsurance'}
                ...
                {/* Suggest Best Policy Search Bar */}
            ...
            {error && (
                <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-md mb-4">
                    <div className="flex">
                        ...
                        <div className="ml-3">
                            <h3 className="text-sm font-medium">Error loading policies</h3>
            ...
            {!loading && !error && plans.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <h3 className="text-xl font-semibold text-gray-800 mb-2">No Matching Policies Found</h3>
                    <p className="text-gray-600 mb-4">
                        {formData 
                            ? "No term insurance policies match your current profile. Please try adjusting your preferences or contact our support team for assistance."
                            : "No term insurance policies are currently available. Please check back later."
                        }
                    </p>
                    {formData && (
                    ...
                    {formData && (
                        <button 
                            onClick={() => {
                                localStorage.removeItem('termInsuranceFormData');
                                window.location.href = '/terminsurance';
                            }}
...
PolicySuggestList.displayName = "PolicySuggestList";
